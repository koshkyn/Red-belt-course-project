...
void DurationFunc() {
	const size_t DOC_COUNT = 10000;
	const size_t WORD_IN_DOC = 100;
	const size_t QUERY_COUNT = 2000;
	const size_t WORD_LENGTH = 1;
	stringstream docs_input;
	stringstream queries_input;
	for(size_t i = 0; i < DOC_COUNT; ++i) {
		for(size_t i2 = 0; i2< WORD_IN_DOC; ++i2) {
			mt19937 rm1(time(0));
			size_t length = rm1() % WORD_LENGTH + 1;
			for(size_t i3 = 1; i3 <= length; ++i3) {
			mt19937 rm(time(0));
			docs_input << ((rm() + i3) % 10);
			}
			docs_input << " ";
	...
	for(size_t i = 0; i < QUERY_COUNT; ++i) {
		...
		size_t length = rm1() % WORD_LENGTH + 1;
		for(size_t i3 = 1; i3 <= length; ++i3) {
			queries_input << ((rm1() + i3) % 10);
		}
		queries_input << "\n";
	}
	//cout << queries_input.str() << endl;
	  SearchServer srv;
	  {
	  ...
	  ostringstream queries_output;
	  {
		LOG_DURATION("AddQueriesStreamThreads");
		for(int i = 5; i < 5; ++i) {
			srv.AddQueriesStream(queries_input, queries_output);
		}
